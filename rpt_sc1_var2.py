...
f = [Paragraph('Расчет токов короткого замыкания', style=st_b),
     ...
     # sp_30,
     fml(f'$x_{{э_\u0020 1}} = \\dfrac {{ x_{{с_\u0020 э1}} \\cdot x_{{Б_\u0020 2}} }} '
         f'{{x_{{с_\u0020 э1}} + x_{{Б_\u0020 2}} }}; \\quad'
         f'x_{{э_\u0020 1}} = \\dfrac {{ {x_n_e[0]:.3f} \\cdot {x_b[0, 1]:.3f} }} '
         f'{{ {x_n_e[0]:.3f} + {x_b[0, 1]:.3f} }} = {x_e[0]:.3f}$'),
     sp_30,
     fml(f'$x_{{К1_\u2211}} = \\dfrac {{ x'
         f''
         f'_{{Б_\u0020 11}} \\left( x_{{АТ_\u0020 э1}} + x_{{э_\u0020 1}} \\right) }} '
         f'{{x_{{э_\u0020 1}} x_{{АТ_\u0020 э1}} + x_{{Б_\u0020 21}} }}; \\quad'
         f'x_{{К1_\u2211}} = \\dfrac {{ {x_b[0, 0]:.3f} \\cdot \\left( {x_at_e:.3f} + {x_e[0]:.3f} \\right)}} '
     ...
     sp_30,
     fml(f'$r_{{э}} = \\dfrac {{r_{{с_\u0020 э}} r_{{Б_\u0020 2}} }} '
         f'{{r_{{с_\u0020 э}} + r_{{Б_\u0020 2}} }}; \\quad'
         f'r_{{э}} = \\dfrac {{ {r_n_e:.3f} \\cdot {r_b[0]:.3f} }} {{ {r_n_e:.3f} + '
         f'{r_b[0]:.3f} }} = {r_e:.3f}$'),
     sp_30,
     # fml(f'$r_{{э_\u0020 2}} = \\dfrac {{r_{{с_\u0020 э}} r_{{Б_\u0020 2}} }} '
     #     f'{{r_{{с_\u0020 э}} + r_{{Б_\u0020 2}} }}; \\quad'
     #     f'r_{{э_\u0020 2}} = \\dfrac {{ {r_n_e[0]:.3f} \\cdot {r_b[1]:.3f} }} '
     #     f'{{{r_b[0]:.3f} + {r_at_e:.3f} {r_e:.3f}}} = {r_e:.3f}$'),
     # sp_30,
     fml(f'$r_{{К1_\u2211}} = \\dfrac {{ r_{{Б_\u0020 1}} \\left( r_{{АТ_\u0020 э}} + r_{{э}} \\right) }} '
         f'{{r_{{Б_\u0020 1}} r_{{АТ_\u0020 э}} + r_{{э}} }}; \\quad'
         f'r_{{К1_\u2211}} = \\dfrac {{ {r_b[0]:.3f} \\cdot \\left( {r_at_e:.3f} + {r_e:.3f} \\right)}} '
         f'{{ {r_b[0]:.3f} + {r_at_e:.3f} + {r_e:.3f} }} = {r_s:.3f}$'),

Table(data = [[Paragraph('Точка кз'), Paragraph('Источник'), fml(f'$I^{{(3)}}_{{п0}}$, кА'), fml(f'$T_a$, с'),
     fml(f'$k_у$'), fml(f'$i_у^{{(3)}}$, кА'), fml(f'$I^{{(1)}}_{{п0}}$, кА')],
     [Paragraph('К1 (РУСН)'), Paragraph('Г1-Г5'), fml(f'${i_sc_3:.3f}$'), fml(f'${cos_g}$'), fml(f'${s_g}$'),
     fml(f'${q_g:.1f}$'), fml(f'${i_g}$')]],
