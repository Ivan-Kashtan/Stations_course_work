...
f = [Paragraph('Расчет токов короткого замыкания', style=st_b),
     ...
     fml(f'$x_{{г1}} = x\'\'_d \\cdot \\dfrac {{S_б}} {{S_{{г_\u0020 ном}} }}; \\quad'
         f'x_{{г1}} = {x_d__:.3f} \\cdot \\dfrac {{{s_b:.0f}}} {{{s_g:.3f}}} = {x_g[0]:.3f}$'),
     sp_20,
     fml(f'$x_{{г2}} = x_2 \\cdot \\dfrac {{S_б}} {{S_{{г_\u0020 ном}} }}; \\quad'
         f'x_{{г2}} = {x2:.3f} \\cdot \\dfrac {{{s_b}}} {{{s_g:.3f}}} = {x_g[1]:.3f}$'),
     sp_20,
     ...
     fml(f'$E_с = {e_s}$'),
     sp_30,
     fml(f'$x_с = \\dfrac {{S_б}} {{S\'\'_кз}}; \\quad x_с = \\dfrac {{{s_b}}} {{{s_sc}}} = {x_s:.3f} $'),
     sp_30,
fml(f'$r_с = {r_n} $'),
sp_30,
fml(f'$x_{{л1}} = x_{{уд}} L_л \\dfrac  {{S_б}} {{U_б^2}}; \\quad '
    f'x_{{л1}} = {ln.x} \\cdot {l_ln} \\dfrac  {{{s_b}}} {{ {u_b[2]}^2 }} = {x_n:.3f} $'),
     ...
     fml(f'$x_{{К1_\u2211}} = \\dfrac {{ x_{{э_\u0020 11}} \\left( x_{{АТ_\u0020 э1}} + x_{{Б_\u0020 21}} \\right) }} '
         ...
         f'x_{{К1 \u2211}} = \\dfrac {{ {x_e1[0]:.3f} \\cdot \\left( {x_at_e:.3f} + {x_e2[0]:.3f} \\right)}} '
     ...
     sp_30,
     fml(f'$r_{{Б_\u0020 3}} = \\dfrac {{r_г + r_{{АТ}} }} {{2}}; \\quad'
         f'r_{{Б_\u0020 3}} = \\dfrac {{{r_g:.3f} + {r_at:.3f} }} {{{n_at}}} = {x_b[0, 1]:.3f}$'),
     sp_30,
     fml(f'$r_{{с_\u0020 э}} = \\dfrac {{r_л}} {{n_л}} + r_с; \\quad'
         f'r_{{с_\u0020 э}} = \\dfrac {{{r_ln:.5f} }} {{{n_ln}}} + {r_n}= {r_n_e:.3f}$'),
     sp_30,
     fml(f'$r_{{АТ_\u0020 э}} = \\dfrac {{r_{{АТ}} }} {{n_{{АТ}} }}; \\quad'
